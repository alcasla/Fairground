...
/// TODO: create_caps for torus
  ...
  if(create_body)
  {
    ...
    for(int j=0; j<circle_sweeps; j++)
    {
      ...
      float next_cosPhi = cos(phi+dphi);
      float next_sinPhi = sin(phi+dphi);
      ...
      float old_x = yPrime*cos(-dsweep);
      float old_y = yPrime*sin(-dsweep);
      float old_z = z;
      ...
      float cosLastSweep = cos(last_sweep);
      float sinLastSweep = sin(last_sweep);
...
//====================================================================
    ...
    if(create_caps)
    {
      /// TODO: make the starting cap
      {
        ...
        float start_sweep = this->getStartSweep();
        float cosStartSweep = cos(start_sweep);
        float sinStartSweep = sin(start_sweep);
